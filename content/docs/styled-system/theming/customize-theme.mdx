...
`variant` or `colorScheme`. Not every component has all three `defaultProps`,
to figure out which prop can be set just have a short look on the theme source
by using the `View theme source` button on the top of every components page.
